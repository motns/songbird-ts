...
function createEndpoint(overrides: {
  ...
  authorizer?: Authorizer<Record<string, never>, Record<string, never>, Record<string, never>, Record<string, never>, AuthOutput, null>,
  endpointConfig?: ReturnType<typeof RestEndpointConfig.create>,
  requestHandler?: (
    req: SanitizedRequest<Record<string, never>, Record<string, never>, Record<string, never>, Record<string, never>, AuthOutput, null>
  ) => Promise<OkResponse<Record<string, never>, Record<string, never>, unknown>>,
  additionalRequestValidators?: RequestValidatorFunction<Record<string, never>, Record<string, never>, Record<string, never>, null>[],
  ...
  return new RestEndpoint({
    ...
    ...overrides.additionalRequestValidators !== undefined && { additionalRequestValidators: overrides.additionalRequestValidators },
    ...overrides.successHttpStatus !== undefined && { successHttpStatus: overrides.successHttpStatus },
  })
}

function rawRequest(overrides: Partial<{
...
describe("RestEndpoint", () => {
  ...
  describe("processRequest", () => {
    it("returns a bad request response when authentication fails validation", async () => {
      ...
      authenticator.result = {
        ...
        validationErrors: {
          header: { errors: [{ code: "invalid_type", message: "Invalid input: expected string, received undefined" }] }
        }
      }
    ...
    it("returns an unauthenticated response when the client is not authenticated", async () => {
      const authenticator = new StubAuthenticator()
      authenticator.result = { isValid: true, isAuthenticated: false, message: "Invalid credentials" }
      const endpoint = createEndpoint({ authenticator })
    ...
    it("returns an unauthorised response when the post-validation authorizer rejects the request", async () => {
      const authorizer = new Authorizer<
        Record<string, never>, Record<string, never>, Record<string, never>, Record<string, never>, AuthOutput, null
      >(async () => false)
    ...
    it("returns an internal error response when the request handler throws", async () => {
      const requestHandler = vi.fn(async (): Promise<OkResponse<Record<string, never>, Record<string, never>, unknown>> => {
    ...
    it("returns a bad request response with merged global errors when additional validators fail", async () => {
      const validatorA: RequestValidatorFunction<Record<string, never>, Record<string, never>, Record<string, never>, null> =
        async () => ({ isValid: false, validationErrors: { errors: [{ code: "custom", message: "Validator A failed" }] } })
      const validatorB: RequestValidatorFunction<Record<string, never>, Record<string, never>, Record<string, never>, null> =
        async () => ({ isValid: false, validationErrors: { errors: [{ code: "custom", message: "Validator B failed" }] } })
    ...
    it("proceeds to the request handler when all additional validators pass", async () => {
      const validator: RequestValidatorFunction<Record<string, never>, Record<string, never>, Record<string, never>, null> =
        async () => ({ isValid: true })
  ...
  describe("openApiDefinition", () => {
    ...
    it("sets security with deduplicated pre-authorizer and authorizer scopes when the authenticator is documented", () => {
      const preAuthorizer = new PreAuthorizer<AuthOutput>(() => true, ["read:users", "shared:scope"])
      const authorizer = new Authorizer<
        Record<string, never>, Record<string, never>, Record<string, never>, Record<string, never>, AuthOutput, null
    ...
    it("includes the request body when the request body reader has an OpenAPI definition", () => {
      ...
      expect(endpoint.openApiDefinition.request?.body).toBe(config.requestBodyReader.openApiDefinition)
    })
    ...
    it("omits success response headers when the schema is empty, and includes content only when the writer documents one", () => {
      const endpoint = createEndpoint()
      const successResponse = endpoint.openApiDefinition.responses[httpStatus.OK] as ResponseConfig | undefined
