...
import {
  DefaultErrorHandler,
  type ErrorHandler,
} from "../../../../../src/libs/webservice/rest/ErrorHandler.js";
...
describe("RestEndpointConfig", () => {
  ...
  describe("noRequestBody", () => {
    it("resets the request body reader to the empty reader", () => {
      const updated = RestEndpointConfig.create().jsonRequestBody(z.object({})).noRequestBody();
  ...
  describe("noResponseBody", () => {
    it("resets to the empty response writer with plain text error writers", () => {
      const updated = RestEndpointConfig.create().jsonResponseBody(z.object({})).noResponseBody();
  ...
  describe("withErrorHandler", () => {
    it("sets a custom error handler and error response schemas", () => {
      ...
      const customHandler: ErrorHandler<{ "x-error-id": string }, Record<string, never>> = {
        ...
        handleInternalError: () => new InternalErrorResponse({ "x-error-id": "1" }, {}),
        handleUnauthenticated: () => new UnauthenticatedResponse({ "x-error-id": "1" }, {}),
        handleUnauthorised: () => new UnauthorisedResponse({ "x-error-id": "1" }, {}),
      };
