...
const endpointConfig =
  RestEndpointConfig.create()
    .route(
      "/greet/:name", // Express style path parameters are supported
      z.object({ // When setting a new route, you need to provide a ZodSchema to validate the path parameters
        name: z.string().min(2).meta({ // Provide OpenAPI metadata
          description: "The name to greet",
          example: "Peter"
        })
      })
    )
    .jsonResponseBody(z.string().meta({
 ...
 */
const restApiBuilder = new RestAPIBuilder(
  "Friendly API",
  "Baby's first Songbird API",
  "1.0.0"
)
