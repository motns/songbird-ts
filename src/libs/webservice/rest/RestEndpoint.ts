...
> {
  ...
  readonly requestHandler: (
    req: SanitizedRequest<
      ...
      RequestReaderOut
    >,
  ) => Promise<OkResponse<SuccessResponseHeaders, SuccessResponseCookies, SuccessResponseWriterIn>>;
   ...
   */
  readonly additionalRequestValidators:
    | RequestValidatorFunction<PathParams, QueryParams, RequestHeaders, RequestReaderOut>[]
    | undefined;
  ...
  constructor({
    ...
    requestHandler: (
      ...
      OkResponse<SuccessResponseHeaders, SuccessResponseCookies, SuccessResponseWriterIn>
    >;
  ...
  > {
    try {
      ...
      const requestSanitizationResult = await this.sanitizeRequest(
        rawRequest,
  ...
  > {
    ...
    const [pathParamsSR, queryParamsSR, headersSR, cookiesSR, requestBodySR] = await Promise.all([
    ...
    const additionalValidatorResults: ComplexValidationResult[] = await Promise.all(
      additionalValidatorPromises,
    );
  ...
  private generateOpenApiDefinition(): RouteConfig {
    ...
    const routeConfig = {
      ...
      request: {
        params: !isEmptySchema(this.config.schemas.pathParams)
          ? this.config.schemas.pathParams
          : undefined,
        query: !isEmptySchema(this.config.schemas.queryParams)
          ? this.config.schemas.queryParams
          : undefined,
        cookies: !isEmptySchema(this.config.schemas.requestCookies)
          ? this.config.schemas.requestCookies
          : undefined,
        headers: !isEmptySchema(this.config.schemas.requestHeaders)
          ? this.config.schemas.requestHeaders
          : undefined,
      ...
      responses: {
        ...
        [httpStatus.BAD_REQUEST.toString()]: {
          description:
            "Returned when the request payload fails to pass validation checks, describing validation errors that occurred",
    ...
    if (this.preAuthorizer || this.authorizer) {
      routeConfig.responses[httpStatus.FORBIDDEN] = {
        description:
          "Returned when the client has successfully authenticated but does not have permission to access this resource",
        ...(this.config.errorMessageWriter.openApiDefinition && {
          content: this.config.errorMessageWriter.openApiDefinition,
        }),
      };
