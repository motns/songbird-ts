...
> extends Authenticator<Output> {
  ...
  async authenticate(req: RawRequest): Promise<AuthenticationResult<Output>> {
    const [pathParamsSR, queryParamsSR, headersSR, cookiesSR] = await Promise.all([
    ...
    const isSanitizationValid =
      pathParamsSR.isValid && queryParamsSR.isValid && headersSR.isValid && cookiesSR.isValid;
    ...
    return this.handler(pathParamsSR.data, queryParamsSR.data, headersSR.data, cookiesSR.data);
  }
}
