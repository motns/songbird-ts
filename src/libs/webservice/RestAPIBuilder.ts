  ...
  addEndpoint(
    ...
    endpoints.forEach((endpoint) => {
      ...
      if (this.endpointByOperationId[endpoint.operationId]) {
        throw new RestAPIBuilderError(`Endpoint with OperationID "${endpoint.operationId}" already exists`)
      }
      ...
      if (endpoint.authenticator.openApiDefinition) {
        if (this.securitySchemeByName[endpoint.authenticator.name]) {
          if (
            this.securitySchemeByName[endpoint.authenticator.name] !== endpoint.authenticator.openApiDefinition
          ) {
    ...
    if (ref) {
      if (this.schemaByRef[ref]) {
        if (this.schemaByRef[ref] !== schema) {
          throw new RestAPIBuilderError(`Schema reference "${ref}" already exists with different Zod Schema`)
        }
      } else {
