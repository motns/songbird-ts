...
export function stripStringPrefix<In extends string, Prefix extends string>(
  input: In,
  prefix: Prefix,
): StripStringPrefix<In, Prefix> {
  const out = input.startsWith(prefix) ? input.substring(prefix.length) : input;
  return out as StripStringPrefix<In, Prefix>;
}

export function stripStringSuffix<In extends string, Suffix extends string>(
  input: In,
  suffix: Suffix,
): StripStringSuffix<In, Suffix> {
...
export function routeConcat<A extends string, B extends string>(
  a: string,
  b: string,
): RouteConcat<A, B> {
